...
public class MedicamentoService {
    ...
    public void initIndexes() {
        try {
            Integer count = jdbcTemplate.queryForObject(
                    "SELECT COUNT(1) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='medicamento' AND index_name='idx_plu_usuario'",
                    Integer.class);
        ...
        try {
            ...
            Integer countUser = jdbcTemplate.queryForObject(
                    "SELECT COUNT(1) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='medicamento' AND index_name='idx_usuario_estado'",
                    Integer.class);
            if (countUser != null && countUser == 0) {
                jdbcTemplate
                        .execute("ALTER TABLE medicamento ADD INDEX idx_usuario_estado (idusuario, estadodelconteo)");
            }

            Integer countDetalle = jdbcTemplate.queryForObject(
                    "SELECT COUNT(1) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='detalleconteo' AND index_name='idx_detalle_usr_fecha'",
                    Integer.class);
    ...
    public void resetStatusByUsuario(Integer idUsuario) {
        jdbcTemplate.update(
                "UPDATE medicamento SET estadodelconteo = 'no' WHERE idusuario = ? AND estadodelconteo = 'sí'",
                idUsuario);
    }
